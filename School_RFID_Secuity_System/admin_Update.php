...
if (isset($_POST['update'])) {
    ...
    if (empty($up_name) || empty($up_email)) {
        header("location: students.php?error=emptyfields");
        exit();
    }
    elseif (!filter_var($up_email,FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z 0-9]*$/", $up_name)) {
        header("location: students.php?error=invalidEN&UN=".$up_name);
        exit();
    }
    ...
    elseif (!preg_match("/^[a-zA-Z 0-9]*$/", $up_name)) {
        header("location: students.php?error=invalidName&E=".$up_email);
        exit();
    }
    else{
        ...
        if ( !mysqli_stmt_prepare($result, $sql)){
            header("location: students.php?error=sqlerror1");
            exit();
        }
        else{
            ...
            if ($row = mysqli_fetch_assoc($resultl)) {
                ...
                if ($pwdCheck == false) {
                    header("location: students.php?error=wrongpasswordup");
                    exit();
                }
                    ...
                    if ($useremail == $up_email) {
                        ...
                        if ( !mysqli_stmt_prepare($result, $sql)){
                            ...
                            if (!$row = mysqli_fetch_assoc($resultl)) {
                                ...
                                header("location: students.php?error=nouser2");
                                exit();
                            }
                        }
                    }
                ...
                header("location: students.php?error=nouser1");
                exit();
            }
        }
    }
